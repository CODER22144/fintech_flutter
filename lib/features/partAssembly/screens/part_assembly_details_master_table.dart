...
class PartAssemblyDetailsMasterTable extends StatefulWidget {
  static String routeName = "PartAssemblyDetailsMasterTable";
  const PartAssemblyDetailsMasterTable({super.key});

  @override
  State<PartAssemblyDetailsMasterTable> createState() =>
      _PartAssemblyDetailsMasterTableState();
}

class _PartAssemblyDetailsMasterTableState
    extends State<PartAssemblyDetailsMasterTable> {
  ...
  void initState() {
    ...
    provider.getPartAssemblyDetailsByMatno();
  }
        ...
        child: SafeArea(
          child: Scaffold(
            appBar: PreferredSize(
                ...
                child: const CommonAppbar(title: 'Assembly Part Details')),
            body: SingleChildScrollView(
              scrollDirection: Axis.vertical,
              child: SingleChildScrollView(
                scrollDirection: Axis.horizontal,
                child: Visibility(
                  visible: provider.partAssemblyDetailsList.isNotEmpty,
                  child: DataTable(
                    columns: const [
                    ...
                    rows: provider.partAssemblyDetailsList.map((data) {
                          ...
                          child: ElevatedButton(
                              onPressed: () async {
                                bool confirmation = await showConfirmationDialogue(
                                    context,
                                    "Do you wish to delete Part Detail: ${data['padId']} ?",
                                    "SUBMIT",
                                    "CANCEL");
                                if (confirmation) {
                                  http.StreamedResponse result =
                                  await provider.deletePartAssemblyDetails('${data['padId'] ?? ""}');
