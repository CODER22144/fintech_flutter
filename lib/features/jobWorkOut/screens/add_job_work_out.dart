...
class AddJobWorkOut extends StatefulWidget {
  static String routeName = "/AddJobWorkOut";

  const AddJobWorkOut({super.key});
  @override
  AddJobWorkOutState createState() => AddJobWorkOutState();
}

class AddJobWorkOutState extends State<AddJobWorkOut> {
    ...
    return Consumer<JobWorkOutProvider>(builder: (context, provider, child) {
      return Scaffold(
        appBar: PreferredSize(
            ...
            child: const CommonAppbar(title: 'Job WorkOut')),
                          ...
                          onPressed: () async {
                            if (formKey.currentState!.validate()) {
                              bool confirmation =
                                  ...
                                  "CANCEL");
                              if (confirmation) {
                                http.StreamedResponse result = await provider.addJobWorkOut();
                            var message = jsonDecode(
                            await result.stream.bytesToString());
                            if (result.statusCode == 200) {
                            context.pushReplacementNamed(AddJobWorkOut.routeName);
