...
class GetClaimReport extends StatefulWidget {
  static String routeName = "/get-claim-report";
  const GetClaimReport({super.key});

  @override
  State<GetClaimReport> createState() => _GetClaimReportState();
}

class _GetClaimReportState extends State<GetClaimReport> {
  @override
  void initState() {
    super.initState();
    TaClaimProvider provider =
        Provider.of<TaClaimProvider>(context, listen: false);
    ...
    return Consumer<TaClaimProvider>(builder: (context, provider, child) {
      return Scaffold(
        appBar: PreferredSize(
            ...
            child: const CommonAppbar(title: 'Initiate TA Claim Report')),
                          ...
                          onPressed: () async {
                            if (formKey.currentState!.validate()) {
                              http.StreamedResponse result =
                                  await provider.getClaimReport();
                              var data = jsonDecode(
                                  await result.stream.bytesToString());
                              if (result.statusCode == 200) {
                                provider.setReport(data);
                                context.pushNamed(ClaimReport.routeName);
