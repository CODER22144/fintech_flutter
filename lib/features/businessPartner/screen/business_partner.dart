...
class BusinessPartner extends StatefulWidget {
  static String routeName = "/businessPartner";
  final String? editing;
  const BusinessPartner({super.key, this.editing});
...
class _BusinessPartnerState extends State<BusinessPartner> {
  late BusinessPartnerProvider provider;
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<BusinessPartnerProvider>(
      ...
      return Scaffold(
        appBar: widget.editing == 'true' ? PreferredSize(
            preferredSize:
            Size.fromHeight(MediaQuery.of(context).size.height * 0.07),
            child: const CommonAppbar(title: 'Edit Business Partner')) : null,
                            ...
                            if (formKey.currentState!.validate()) {
                              ...
                              if (confirmation) {
                                ...
                                if (result.statusCode == 200) {
                                  if (widget.editing == "true") {
                                    context.pop();
                                    context.pushReplacementNamed(GetBusinessPartner.routeName);
                                  } else {
                                    context.pushReplacementNamed(
                                        BusinessPartner.routeName);
                                  }
